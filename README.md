...
#### Application of Continuous Execution for Transact-Ion

To be able to couple trade agreements between chains we require a different mechanism to a simple nonce to identify trade agreements which would only be able to uniquely identify the trade agreement within the chain it was created on. Due to the fact that we bind two cross-chain interactions to the same trade agreement via usage of the same trade agreement id being derived from the id used by the initiator this results in any counterparty chain being open to id clashes if agreements are being formed with other chains that would also use its own set of nonces. Therefore we must find a unique way to identify each trade agreement/intent such that they are universally unique.

Thus we utilise the lessons from `Continuous Execution Theory` to set up universally unique identifiers for the chain of execution we are constructing